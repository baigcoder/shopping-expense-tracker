        ...
        try {
            ...
            const websiteTabs = tabs.filter(tab =>
                tab.url && (
                    ...
                    tab.url.includes('vibe-tracker') ||
                    tab.url.includes('vibetracker')
                )
            );
                ...
                try {
                    await chrome.scripting.executeScript({
                        ...
                        func: (userEmail) => {
                            ...
                            localStorage.setItem('vibe_tracker_extension_synced', JSON.stringify({
                                synced: true,
                                email: userEmail,
                                timestamp: Date.now()
                            }));
                            // Also set the real-time flags
                            localStorage.setItem('vibe_tracker_extension', JSON.stringify({
                                installed: true,
                                version: '2.9.0',
                                timestamp: Date.now()
                            }));
                            localStorage.setItem('vibe_tracker_extension_auth', JSON.stringify({
                                loggedIn: true,
                                email: userEmail,
                                timestamp: Date.now()
                            }));
                            console.log('✅ Extension sync flag injected via popup!');
