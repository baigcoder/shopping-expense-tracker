...
(function () {
    ...
    function showNotification(store, amount, isManual = false) {
        ...
        // Build notification using DOM methods for security
        const wrapper = document.createElement('div');
        wrapper.className = 'et-notification';
        ...
        iconCircle.style.cssText = `
            ...
            font-size: 1.25rem;
        `;
        iconCircle.textContent = isManual ? '📌' : '✓';

        const textDiv = document.createElement('div');
        const strongEl = document.createElement('strong');
        strongEl.style.cssText = 'display: block; font-size: 0.9rem;';
        strongEl.textContent = isManual ? 'Clipped!' : 'Purchase Tracked!';
        const spanEl = document.createElement('span');
        spanEl.style.cssText = 'font-size: 0.8rem; color: #666;';
        spanEl.textContent = `${store} - $${amount.toFixed(2)}`;
        textDiv.appendChild(strongEl);
        textDiv.appendChild(spanEl);

        const closeBtn = document.createElement('button');
        closeBtn.className = 'et-close';
        ...
        wrapper.appendChild(iconCircle);
