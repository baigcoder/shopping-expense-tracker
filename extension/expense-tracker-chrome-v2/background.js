...
async function handleSubscriptionDetected(data) {
    ...
    try {
        ...
        if (data.is_trial && data.trial_days > 0) {
            trialStartDate = today.toISOString().split('T')[0];
            const endDate = new Date(today);
            endDate.setDate(endDate.getDate() + data.trial_days);
...
function showSubscriptionNotification(data) {
    ...
    try {
        chrome.notifications.create(notificationId, {
            ...
            title: data.is_trial ? '⏰ Trial Started!' : '📦 Subscription Added!',
            message: data.is_trial
                ? `${data.name} - ${data.trial_days} days free trial. We'll remind you before it ends!`
                : `${data.name} - Now tracking your subscription.`,
            priority: 2
        });
